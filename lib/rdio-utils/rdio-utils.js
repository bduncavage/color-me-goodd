...
(function(R) {
  ...
  window.rdioUtils = {
    ...
    // ----------
    albumWidget: function(album, alternativeColor) {
      return new this.AlbumWidget(album, alternativeColor);
    },
...
(function(R, rdioUtils) {
  ...
  // ----------
  rdioUtils.AlbumWidget = function(album, alternativeColor) {
    var i;
    ...
    var fillColor = album.dominantColor;
    if (typeof alternativeColor === 'undefined') {
      fillColor = alternativeColor;
    }
    var html = ''
        + '<div class="rdio-utils-album-cover">'
          + '<a href="http://www.rdio.com' + rdioUtils._escape(album.url) + '">'
            + '<div class="rdio-utils-album-icon"' +
                'style="background-color: rgb(' + fillColor.r + ',' + fillColor.g + ',' + fillColor.b + ')' + '"></div>'
...
(function(R, rdioUtils) {
  ...
  rdioUtils.CollectionTracker = function(config) {
    ...
    this._bigExtras = '-*,playCount,backgroundImageUrl,bigIcon,dominantColor,releaseDate,duration,isClean,canStream,icon,'
  ...
  rdioUtils.CollectionTracker.prototype = {
    ...
    _startLoad: function() {
      ...
      if (this._firstTime) {
        this._count = 200;
        this._extras = this._bigExtras + ',albumKey,rawArtistKey,dominantColor,bigIcon';
      } else {
        this._count = 1000;
    ...
    _getAlbums: function(keys, callback) {
      ...
      this._loading.request = R.request({
        ...
        content: {
          keys: keysChunk.join(','),
          extras: this._bigExtras + ',key,artistKey,dominantColor,backgroundImageUrl'
        },
        success: function(data) {
          self._loading.request = null;
          var addedAlbums = [];
